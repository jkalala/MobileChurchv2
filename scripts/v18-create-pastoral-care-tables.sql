-- Create pastoral care management tables
...
CREATE TABLE IF NOT EXISTS counseling_sessions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    member_id UUID REFERENCES members(id) ON DELETE CASCADE,
    counselor_id UUID REFERENCES members(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_care_records_member_id ON care_records(member_id);
CREATE INDEX IF NOT EXISTS idx_care_records_assigned_to ON care_records(assigned_to);
CREATE INDEX IF NOT EXISTS idx_care_records_status ON care_records(status);
CREATE INDEX IF NOT EXISTS idx_care_records_priority ON care_records(priority);
CREATE INDEX IF NOT EXISTS idx_care_records_scheduled_date ON care_records(scheduled_date);
...
CREATE INDEX IF NOT EXISTS idx_prayer_requests_assigned_to ON prayer_requests(assigned_to);

CREATE INDEX IF NOT EXISTS idx_crisis_alerts_member_id ON crisis_alerts(member_id);
...
CREATE INDEX IF NOT EXISTS idx_crisis_alerts_assigned_to ON crisis_alerts(assigned_to);

CREATE INDEX IF NOT EXISTS idx_counseling_sessions_member_id ON counseling_sessions(member_id);
CREATE INDEX IF NOT EXISTS idx_counseling_sessions_counselor_id ON counseling_sessions(counselor_id);
CREATE INDEX IF NOT EXISTS idx_counseling_sessions_scheduled_date ON counseling_sessions(scheduled_date);
...
INSERT INTO care_team_members (member_id, role, specializations, availability, max_weekly_visits, contact_preferences, languages) VALUES
(
