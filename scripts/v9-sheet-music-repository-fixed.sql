-- Sheet Music Repository Database Schema (Fixed Version)
...
-- Insert music categories and store their IDs
DO $$
DECLARE
    hymns_id UUID;
...
BEGIN
    ...
    ('Hymns', 'Traditional church hymns', '#8B5CF6') RETURNING id INTO hymns_id;
    
    INSERT INTO music_categories (name, description, color) VALUES
    ('Contemporary', 'Modern worship songs', '#10B981') RETURNING id INTO contemporary_id;
    
    INSERT INTO music_categories (name, description, color) VALUES
    ('Gospel', 'Gospel and spiritual songs', '#F59E0B') RETURNING id INTO gospel_id;
    
    INSERT INTO music_categories (name, description, color) VALUES
    ('Christmas', 'Christmas and Advent music', '#EF4444') RETURNING id INTO christmas_id;
    
    INSERT INTO music_categories (name, description, color) VALUES
    ('Easter', 'Easter and resurrection songs', '#F97316') RETURNING id INTO easter_id;
    
    INSERT INTO music_categories (name, description, color) VALUES
    ('Praise', 'Praise and celebration songs', '#06B6D4') RETURNING id INTO praise_id;
    
    INSERT INTO music_categories (name, description, color) VALUES
    ('Communion', 'Songs for communion service', '#84CC16') RETURNING id INTO communion_id;
    
    INSERT INTO music_categories (name, description, color) VALUES
    ('Prayer', 'Prayer and meditation songs', '#6366F1') RETURNING id INTO prayer_id;

    -- Insert sample sheet music using the category IDs
    ...
    ('Go Tell It on the Mountain', 'Traditional Spiritual', gospel_id, 'G Major', 'Beginner', ARRAY['Soprano', 'Alto', 'Tenor', 'Bass'], '/music/sheets/go-tell-it.pdf', '/music/audio/go-tell-it.mp3', 'Go tell it on the mountain, over the hills and everywhere...', true, ARRAY['christmas', 'evangelism', 'joy']),
