-- Fix streaming tables with proper stream key generation and single row handling
-- This script fixes the "query returned more than one row" error
...
-- Insert sample streaming data with proper stream keys and LIMIT clauses
DO $$
DECLARE
    sample_user_id UUID;
    sample_member_id UUID;
    stream_1_id UUID := gen_random_uuid();
    stream_2_id UUID := gen_random_uuid();
BEGIN
    -- Get a sample user ID with LIMIT 1
    SELECT id INTO sample_user_id FROM auth.users LIMIT 1;
    
    -- Get a sample member ID with LIMIT 1
    ...
    -- Create a sample member if none exists
    IF sample_member_id IS NULL THEN
        INSERT INTO members (id, name, email, phone, status, created_at, updated_at)
        VALUES (
            gen_random_uuid(),
            'Administrador Streaming',
            'streaming@church.com',
            '+244900000000',
            'active',
            NOW(),
    ...
    -- Insert sample streaming sessions with explicit IDs
    ...
    -- Insert sample chat messages using explicit stream IDs
    INSERT INTO stream_chat_messages (
        stream_id, user_id, member_id, user_name, message, message_type
    ) VALUES 
    (
        stream_2_id,
        sample_user_id,
    ...
    -- Insert sample prayer requests using explicit stream ID
    INSERT INTO stream_prayer_requests (
        stream_id, requester_id, requester_name, prayer_request, prayer_count
    ) VALUES 
    (
        stream_2_id,
        sample_member_id,
    ...
    -- Insert sample donations using explicit stream ID
    INSERT INTO stream_donations (
        stream_id, donor_id, donor_name, amount, currency, message, is_anonymous
    ) VALUES 
    (
        stream_2_id,
        sample_member_id,
    ...
    -- Insert sample analytics using explicit stream ID
    ...
    -- Insert sample viewers using explicit stream ID
    INSERT INTO stream_viewers (
        stream_id, user_id, member_id, device_type, location_country, location_city, 
    ...
    -- Insert sample hybrid service
    INSERT INTO hybrid_services (
        event_id, stream_id, in_person_capacity, in_person_registered, 
        in_person_attended, online_capacity, online_registered, online_attended,
        registration_required, check_in_enabled, hybrid_features
    ) VALUES 
    (
        ...
        false,
        true,
        '{"qr_checkin": true, "live_chat": true, "prayer_requests": true, "donations": true}'
    );

    -- Insert sample moderator
    INSERT INTO stream_moderators (
        stream_id, moderator_id, permissions, assigned_by
    ) VALUES 
    (
        stream_1_id,
        sample_member_id,
        '{"can_delete_messages": true, "can_timeout_users": true, "can_highlight_messages": true, "can_manage_donations": true}',
        sample_user_id
    );
...
-- Create a function to get streaming statistics
CREATE OR REPLACE FUNCTION get_streaming_stats(stream_id_param UUID DEFAULT NULL)
RETURNS TABLE (
...
BEGIN
    ...
    SELECT 
        ...
        COUNT(CASE WHEN status = 'live' THEN 1 END)::INTEGER as active_streams,
        COALESCE(SUM(viewer_count), 0)::INTEGER as total_viewers,
        (SELECT COUNT(*)::INTEGER FROM stream_chat_messages 
         WHERE stream_id_param IS NULL OR stream_id = stream_id_param) as total_messages,
        (SELECT COALESCE(SUM(amount), 0) FROM stream_donations 
         WHERE stream_id_param IS NULL OR stream_id = stream_id_param) as total_donations,
        (SELECT COUNT(*)::INTEGER FROM stream_prayer_requests 
         WHERE stream_id_param IS NULL OR stream_id = stream_id_param) as total_prayers
    FROM streaming_sessions
    WHERE stream_id_param IS NULL OR id = stream_id_param;
END;
$$ LANGUAGE plpgsql;
